    ...
    <div className='w-full h-full flex flex-col justify-center items-center'>
      <div className='flex bg-white w-full items-center justify-between px-32 py-4 border-b-2 shadow-sm shadow-gray-100 border-gray-200'>
        ...
        <div className='flex gap-4 items-center'>
          ...
          <Link to="/dashboard/jobs/new"><button className='rounded-full px-4 py-1 text-sm bg-[#00756A] text-white'>Create a New Job</button></Link>
        </div>
      </div>
      {
        !loading
          ...
          <div className='w-full min-h-[78vh] grid grid-flow-row px-32 gap-8 py-10'>
            {
              jobList.length
                ...
                jobList.map((item, index) => {
                  return <Job key={item.id} job={item} onUploadClick={handleUploadClick} />
                })
                :
                // <></>
      ...
      {showUploadForm && (
       <div className="fixed inset-0 bg-black bg-opacity-40 z-50 overflow-y-auto flex justify-center items-center px-4">
        <div className="max-h-[90vh] overflow-y-auto bg-white rounded-lg p-6 w-full max-w-4xl">
          <UploadCandidateForm
            onClose={closeUploadForm}
            onSubmit={(data) => {
              console.log("Data submitted from child:", data);
            }}
          />
        </div>
      </div>
